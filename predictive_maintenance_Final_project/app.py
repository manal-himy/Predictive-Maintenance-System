...
def download_results():
    try:
        # 1. استقبال البيانات النصية (JSON) من المتصفح
        data_json = request.form.get('table_data')

        # 2. تحويل النص إلى جدول بيانات (DataFrame)
        # نستخدم StringIO هنا لأن البيانات قادمة كنص JSON
        df = pd.read_json(io.StringIO(data_json))

        # 3. إنشاء وعاء بايتات فارغ في الذاكرة
        proxy = io.BytesIO()

        # 4. حفظ الجدول داخل الوعاء مع ترميز 'utf-8-sig'
        # هذا السطر هو الذي يحل مشكلة "الطلاسم" ويظهر العربي بوضوح في إكسل
        df.to_csv(proxy, encoding='utf-8-sig', index=False)

        # 5. العودة لبداية الملف لإرساله
        proxy.seek(0)

        # 6. إرسال الملف مباشرة دون الحاجة لعمل encode إضافي
        return send_file(
            proxy,
            mimetype='text/csv',
            as_attachment=True,
        ...
        # إذا حدث خطأ سيطبع لك السبب في الـ Terminal
        print(f"Error: {e}")
